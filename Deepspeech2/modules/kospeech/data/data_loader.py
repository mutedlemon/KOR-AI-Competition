...
from modules.kospeech.data import load_dataset
from modules.kospeech.utils import logger
from modules.kospeech.data import SpectrogramParser
...
class SpectrogramDataset(Dataset, SpectrogramParser):
    ...
    def _augment(self, spec_augment):
        """ Spec Augmentation """
        if spec_augment:
            print("Applying Spec Augmentation in kospeech module data_loader")
...
def split_dataset(config, transcripts_path: str, vocab: Vocabulary):
    """
    ...
    #     valid_num = 5567
    # else:
    #     raise NotImplementedError("Unsupported Dataset : {0}".format(config.dataset))
    ...
    valid_time_step = math.ceil(len(valid_audio_paths) / config.batch_size)
    train_time_step = total_time_step - valid_time_step

    print(total_time_step, config.batch_size)
    ...
    train_num_per_worker = math.ceil(train_num / config.num_workers)
    ...
    for idx in range(config.num_workers):
        ...
        print(config.transform_method, config.audio_extension)
